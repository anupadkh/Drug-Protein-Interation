...
def combination_feature_1d(FLAGS, FEATURE_LENGTH, NUM_FILTERS, FILTER_LENGTH, FILTERS_EXTENSION):
    XEInput = Input(shape=(FEATURE_LENGTH,), dtype='float32')
    XYInput = Reshape(target_shape=(FEATURE_LENGTH,1), input_shape=(FEATURE_LENGTH,) )(XEInput)
    energy_mat = Conv1D(filters = NUM_FILTERS, kernel_size=FILTER_LENGTH,  activation='relu', padding='valid')(XYInput)
    for x in range(FILTERS_EXTENSION-1):
        energy_mat = Conv1D(filters=NUM_FILTERS*(x+2), kernel_size=FILTER_LENGTH,  activation='relu', padding='valid')(energy_mat)
    encode_energy = GlobalMaxPooling1D()(energy_mat)
        ...
        for param1ind in range(len(paramset1)): #hidden neurons
            ...
            for param2ind in range(len(paramset2)): #learning rate
                ...
                for param3ind in range(len(paramset3)):
                    ...
                    my_callbacks = checkpoints.checkpoint_with_fname(str(FLAGS.method) + "model"+ '{param1value}-{param2value}-{param3value}.h5'.format(param1value=param1value,param2value = param2value,param3value = param3value))
                    ...
                    if type(gridmodel) != eval("tuple"):
                        ...
                        # gridmodel.save_weights(str(FLAGS.method)+"model.h5")
