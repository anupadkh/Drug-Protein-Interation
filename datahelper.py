...
class DataSet(object):
	def __init__(self, fpath, setting_no, seqlen, smilen, need_shuffle = False):
		...
		mx = msg.make_feature_set(proteins)
		bi_gram = dict(zip(mx[2], np.zeros(len(mx[2]))))
		self.contact_matrix = pd.read_csv('R2RSRM.csv', index_col = 0)
		self.mx = mx
	...
	def get_energy(self):
		XT2 = []:
		for t in self.proteins.keys():
			XT2.append(self.turn_energy_set(self.proteins[t]))
		return XT2
	...
	# Anup Feature Set using NGSG


	def turn_energy_set(self, prot):
		feat = msg.feat_count(key='abc', seq = {'abc':prot}, table=self.bi_gram, buffer=1)
		feat = {x:feat[x] for x in msg.removed_unrequired_aminos(self.mx[0])}
		bi_gram_contact = msg.get_contact_mappings(self.mx[0],self.contact_matrix)
		y = sorted(list(feat), key = lambda x:x[1])
		m = [0 if (bi_gram_contact[0][r] == 0) else bi_gram_contact[0][r] for r in y]
		m.reverse()
		return m
